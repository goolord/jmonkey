...
interpretJStat (Free (Toggle (Elem t) (Class s) n)) = [jmacro| `jref t`.classList.toggle(`s`); |] <> interpretJStat n
interpretJStat (Free (Toggle (Elem t) sel@(Id _) n)) = [jmacro| 
  if (`jref t`.id == `toStr sel`) { 
    `jref t`.id = ""; 
  } else { 
    `jref t`.id = `toStr sel`;
  }
  |] <> interpretJStat n
interpretJStat (Free (Toggle (Elems t) (Class s) n)) = [jmacro|
  `jref t`.forEach( function e {
    e.classList.toggle(`s`);
  })
  |] <> interpretJStat n
interpretJStat (Free (Toggle (Elems _) (Id _) n)) = forbidden <> interpretJStat n        
